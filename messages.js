...
document.addEventListener('DOMContentLoaded', () => {
  ...
  onAuthStateChanged(auth, async (user) => {
    ...
    // Check if opening a specific convo from URL
...
function initMessagesUI() {
  document.getElementById('messages-auth-required')?.remove();
  const root = document.getElementById('messages-root');
...
function loadConversations() {
  ...
  _unsubConvos = onSnapshot(q, async (snap) => {
    ...
    for (const d of snap.docs) {
      ...
      const item = await buildConvoItem(convo);
...
async function buildConvoItem(convo) {
  ...
  if (isGroup) {
    ...
    const otherProfile = await getProfile(otherUid);
    name = otherProfile?.displayName || otherProfile?.username || 'Unknown';
    avatarHTML = otherProfile?.avatarURL
      ? `<img src="${otherProfile.avatarURL}" style="width:44px;height:44px;border-radius:50%;object-fit:cover;flex-shrink:0;">`
  ...
  item.innerHTML = `
    ...
    <div style="flex:1;min-width:0;">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;">
        <span style="font-size:14px;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${name}</span>
      ...
      <div style="font-size:12px;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;margin-top:2px;">${escapeHtml(lastMsg)}</div>
    </div>
  `;
...
async function openConversation(convoId, name, isGroup) {
  ...
  if (!_disclaimerShown && !localStorage.getItem('flux_dm_disclaimer')) {
    ...
    <div style="display:flex;align-items:center;justify-content:center;height:100%;padding:32px;">
      <div style="max-width:400px;text-align:center;">
        ...
        <div style="display:flex;gap:10px;justify-content:center;">
          <button id="disclaimer-accept" style="padding:10px 24px;background:var(--accent);color:white;border:none;border-radius:10px;font-weight:700;cursor:pointer;font-size:14px;">I Understand</button>
          <a href="messages.html" style="padding:10px 24px;background:transparent;border:1px solid var(--glass-border);color:var(--muted);border-radius:10px;font-weight:600;cursor:pointer;font-size:14px;text-decoration:none;display:flex;align-items:center;">Go Back</a>
        </div>
      </div>
    </div>
  `;
...
function loadConversationMessages(convoId, name, isGroup) {
  ...
  panel.innerHTML = `
    <div class="chat-header-bar">
      ...
      ${isGroup ? `<button id="group-info-btn" style="background:none;border:none;cursor:pointer;font-size:18px;color:var(--muted);">ℹ️</button>` : ''}
    </div>
  ...
  document.getElementById('back-btn').addEventListener('click', () => {
    panel.innerHTML = '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--muted);font-size:14px;">Select a conversation</div>';
    _activeConvoId = null;
  ...
  _unsubMessages = onSnapshot(q, (snap) => {
    ...
    list.innerHTML = '';
    snap.docs.forEach(d => {
      list.appendChild(renderMessage({ id: d.id, ...d.data() }));
    });
...
async function sendMessage() {
  ...
  try {
    ...
    if (convoSnap.exists()) {
      ...
      const unreadUpdate = {};
      members.forEach(uid => {
        if (uid !== _currentUser.uid) unreadUpdate[`unread.${uid}`] = (convoSnap.data().unread?.[uid] || 0) + 1;
      });
...
/* ── Open DM with username (from URL) ── */
...
async function startDM(targetUid, targetProfile) {
  ...
  snap.docs.forEach(d => {
    const data = d.data();
    if (data.members.includes(targetUid)) existingId = d.id;
  });

  if (existingId) {
    openConversation(existingId, targetProfile.displayName || targetProfile.username, false);
    return;
  }
...
function showNewChatModal() {
  ...
  modal.innerHTML = `
    <div style="background:var(--panel);border-radius:20px;padding:28px;width:100%;max-width:380px;box-shadow:0 30px 80px rgba(0,0,0,0.2);position:relative;">
      ...
      <div id="new-chat-results" style="max-height:240px;overflow-y:auto;display:flex;flex-direction:column;gap:4px;"></div>
    </div>
  `;
    ...
    timer = setTimeout(async () => {
      ...
      results.filter(p => p.uid !== _currentUser.uid).forEach(p => {
        ...
        item.innerHTML = `
          ...
          <div>
            <div style="font-size:13px;font-weight:700;color:var(--text);">${p.displayName || p.username}</div>
...
function showNewGroupModal() {
  ...
  const selectedMembers = new Map(); // uid -> profile
  ...
  modal.innerHTML = `
    <div style="background:var(--panel);border-radius:20px;padding:28px;width:100%;max-width:400px;box-shadow:0 30px 80px rgba(0,0,0,0.2);position:relative;max-height:90vh;overflow-y:auto;">
      ...
      <div id="group-search-results" style="max-height:160px;overflow-y:auto;margin-bottom:8px;"></div>
      <div id="group-members-chips" style="display:flex;flex-wrap:wrap;gap:6px;min-height:32px;margin-bottom:12px;"></div>
  ...
  const updateChips = () => {
    ...
    selectedMembers.forEach((p, uid) => {
      ...
      chip.innerHTML = `@${p.username} <button style="background:none;border:none;color:rgba(255,255,255,0.8);cursor:pointer;font-size:12px;padding:0;margin-left:2px;" data-uid="${uid}">✕</button>`;
  ...
  document.getElementById('group-search').addEventListener('input', (e) => {
    ...
    timer = setTimeout(async () => {
      ...
      results.filter(p => p.uid !== _currentUser.uid).forEach(p => {
        ...
        item.innerHTML = `
          <div style="font-size:13px;font-weight:700;color:var(--text);">@${p.username}</div>
          <div style="font-size:11px;color:var(--muted);">${p.displayName || ''}</div>
          ${selectedMembers.has(p.uid) ? '<span style="margin-left:auto;color:#22c55e;font-size:12px;">✓ Added</span>' : ''}
        `;
        item.addEventListener('mouseenter', () => item.style.background = 'var(--bg)');
        item.addEventListener('mouseleave', () => item.style.background = '');
